...
unsigned short
updcrc(register int c, register unsigned crc) // cyclic redundancy check
{
	register int count;
...
crc16ns (uint16_t* crc16nsP, uint8_t* buf)
{
	 ...
	 //if (Crcflg) {
       ...
       //  *crc16nsP = 0;
		 /// changed to oldcrc
		 /// use htons() to change host byte to network byte for sending data blocks
		 *crc16nsP = htons(oldcrc);
     //}
	 //else
		 //sendline(checksum);
}
